...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{		
		cerr <<  "Could not open texture file" <<  filename << "\n";
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		cerr << "Could not read file header" << filename << "\n";
	...
	{
		cerr << "TGA file be type 2 or type 10 " << filename << "\n";
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{										
		cerr << "Could not read info header" << "ERROR" << "\n";
	...
	{
		cerr << "Invalid texture information" << filename << "\n";
	...
	{
		cerr << "Could not allocate memory for image" << filename << "\n";
	...
	{
		cerr << "Could not read image data" << "ERROR" << "\n";
...
{ 
	...
	do
	{
		...
		{
			cerr << "Could not read RLE header" << filename << "\n";
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					cerr << "Could not read image data" << filename << "\n";
				...
				{
					cerr << "Too many pixels read" << filename << "\n";

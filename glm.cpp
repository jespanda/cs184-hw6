...
GLvoid glmDraw(GLMmodel* model, GLuint mode,char *drawonly)
{
    ...
    while (group) 
	{
     ...
     //cout << "Material " << group->material <<  " props\n";
  //*
  float emission[4] = { 0.0, 0.0, 0.0, 0.0 };
  glUniform4fv( ambientcol, 1, material->ambient );
  glUniform4fv( diffusecol, 1, material->diffuse );
  glUniform4fv( specularcol, 1, material->specular );
  glUniform4fv( emissioncol, 1, emission );//material->emission );
  glUniform1fv( shininesscol, 1, &material->shininess );
  //*/
        }
